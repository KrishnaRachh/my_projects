...
realized_variance = df_data[index_list[0]].iloc[:,1]
...
am_garch = arch_model(returns, vol='Garch', p=1, o=1, q=1, dist="Normal") # add for t-distribution  dist="StudentsT"
res_garch = am_garch.fit(update_freq=2)
print(res_garch.summary())
...
am_garch = arch_model(returns, vol='Garch', p=1, o=1, q=1, power=1 , dist="Normal") # add for t-distribution  dist="StudentsT"
res_garch = am_garch.fit(update_freq=2)
print(res_garch.summary())
